...
LINEAR_POROGRAMS_DIR = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '..', 'LinearPrograms'))
...
def generate_random_2d_feasible_linear_program(num_of_constraints: int,  max_value: int = 100) -> Tuple[ObjectiveFunction, List[Constraints]]:
    ...
    while len(cons) < num_of_constraints:
        c = Constraints(a=random.randint(1, max_value), b=random.randint(
            1, max_value), lessOrGreater=GreaterOrLess.LESS, c=random.randint(10*max_value, 10*max_value*max_value))
        ...
        if (c.contains(p1) or c.contains(p2) or c.contains(p3)):
            ...
            if (c.contains(p1) or c.contains(p2) or c.contains(p3)):
                ...
                print(
                    "unlucky, you will missing a constraint, but this happens rarely, so it is ok")
...
def generate_to_file(num_of_constraints: int,  max_value: int = 100, file_name: str = "") -> str:
    ...
    files = [f for f in os.listdir(
        LINEAR_POROGRAMS_DIR) if f.startswith("bounded_problem")]
    if file_name == "":
        file_name = "bounded_problem" + \
            str(int(files.pop().replace("bounded_problem", ""))+1)
